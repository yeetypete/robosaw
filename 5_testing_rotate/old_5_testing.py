...
def cut(model,caps):
    ...
    #if not GPIO.input(cut_btn): # indent if uncommented later
    print("\nCut initiated. GTFO!")
...
def run(model,caps):
    ...
    try:
        ...
        caps[2].release()
        model.cut_flag = True # Once ready to cut raise the cut flag
...
model,caps = initialize() # global model and caps
if __name__ == "__main__":
    ...
    GPIO.add_event_detect(run_btn, GPIO.FALLING, 
            callback=run_btn_callback, bouncetime=500)

    GPIO.add_event_detect(cut_btn, GPIO.FALLING, 
            callback=cut_btn_callback, bouncetime=500)

    # interrupt to feed the wood manually
    GPIO.add_event_detect(eject_btn, GPIO.FALLING, 
            callback=eject_btn_callback, bouncetime=500)
